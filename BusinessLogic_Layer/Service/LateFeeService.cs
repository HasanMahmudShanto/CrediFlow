            ...
            foreach(var loan in CustomerLoans)
            {
                if(loan.Status == "Active" && DateTime.Now > loan.Next_Installment_Date && loan.CustomerDTO.Status != "Terminated") //Late
                {
                    ...
                    NotificationDTO notification = new NotificationDTO
                    {
                        ...
                        Message = $"A late fee of {Late_Fee:C} has been applied to your {loan.LoanDTO.Loan_Type} with ID:{loan.Customer_Loan_Id} with  due to a missed installment payment. Please make the payment at your earliest convenience to avoid further penalties.",
                        Date = DateTime.Now,
                        Is_Read = false,
